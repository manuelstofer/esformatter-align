...
function alignTokens(tokens) {
  var alignToCol = Math.max.apply(Math, tokens.map(getTokenColumn));
  tokens.forEach(function(token) {
    var whitespace = token.prev;
    if (!isWhiteSpace(whitespace)) {
      throw new Error('no whitespace before token to align');
    }
    var whitespaceDiff   = alignToCol - getTokenColumn(token);
    var whitespaceLength = whitespace.value.length + whitespaceDiff;
    whitespace.value = repeat(' ', whitespaceLength);
  });
}

function getTokenColumn(token) {
